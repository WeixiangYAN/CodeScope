...
def parse_arguments():
    ...
    parser.add_argument('--result_save_name', default='program_synthesis_eval_starcoder.jsonl',type=str)
    parser.add_argument('--log_file_name', default='program_synthesis_eval_starcoder.logs',type=str),
...
7. Target programming language: {lang} 
8. Choose one programming language version from: {env_map[lang.lower()]}

Respond should only with a string in the following JSON format:
[{{"version": specific version used for {lang}, "target code":  the code you produced in the respective programming language version."}}] """
